export default async function handler(req, res) {
  try {
    ...
    const payload = { query:q, max_results:6, include_answer:false, search_depth:"advanced", topic:"news", time_range:"7d" };
    ...
    }).map(it => ({ ...it, _score:(it.published?2:0)+(it.host?1:0)+(/\bmortgage|rate|treasury|mbs/i.test((it.title+" "+it.snippet))?2:0) }))
      .sort((a,b)=> b._score-a._score).slice(0,5).map(({_score,...rest})=>rest);
