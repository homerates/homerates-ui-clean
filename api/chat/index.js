...
async function handler(req, res) {
  ...
  try {
    ...
    // Safer URL base to avoid host edge cases
    ...
    if (req.method !== "POST") return send(res, 405, { ok:false, reason:"method-not-allowed", method:req.method });

    const body = await readJson(req);
    const userMsg = body?.messages?.[0]?.content || body?.prompt || "";

    if (mode === "openai") return send(res, 200, await callOpenAI(userMsg));
